<section id="ubicaciones" class="tab-content closed px-3">
    <main class="table-data row relative mx-md-0 hidden">
        <header class="content-header col-12 mx-lg-0 p-md-0">
            <h2 class="MontereyFLF title text-uppercase text-left mt-4 mb-3 mt-md-0 px-2">Ubicaciones</h2>
        </header>
        <main class="content-table col-12 p-md-0">
            <table id="ubicaciones-table" class="table">
                {{-- <thead>
                    <tr class="background background-uno color-tres">
                        <th scope="col" class="Work-Sans h4">Id</th>
                        <th scope="col" class="Work-Sans h4">Nombre</th>
                        <th scope="col" class="Work-Sans h4">Última vez actualizada</th>
                        <th scope="col"></th>
                    </tr>
                </thead>
                <tbody>
                    <tr class="adding">
                        <td class="td-id_location" scope="row">4</td>
                        <td class="td-name">
                            <input type="text" name="name" id="name" placeholder="Nombre" class="form-input p-2">
                            <div class="d-block">
                                <span class="support support-box support-name hidden"></span>
                            </div>
                        </td>
                        <td class="td-updated_at">En creación...</td>
                        <td class="actions px-0">
                            <a href="#" class="btn btn-uno-transparent btn-icon mr-1" title="Aceptar">
                                <i class="icon fas fa-check"></i>
                            </a>
                            <a href="#" class="btn btn-uno-transparent btn-icon mr-md-1" title="Cancelar">
                                <i class="icon fas fa-times"></i>
                            </a>
                        </td>
                    </tr>
                    <tr>
                        <td class="td-id_location" scope="row">1</td>
                        <td class="td-name">
                            <input type="text" name="name" id="name" placeholder="Nombre" value="pepe" disabled class="form-input p-2">
                            <div class="d-block">
                                <span class="support support-box support-name hidden"></span>
                            </div>
                        </td>
                        <td class="td-updated_at">18/10/2020</td>
                        <td class="actions px-0">
                            <a href="#" class="btn btn-uno-transparent btn-icon mr-1" title="Actualizar">
                                <i class="icon fas fa-pen"></i>
                            </a>
                            <a href="#" class="btn btn-uno-transparent btn-icon mr-md-1" title="Borrar">
                                <i class="icon fas fa-trash"></i>
                            </a>
                        </td>
                    </tr>
                    <tr>
                        <td class="td-id_location" scope="row">2</td>
                        <td class="td-name">
                            <input type="text" name="name" id="name" placeholder="Nombre" value="pepe" disabled class="form-input p-2">
                            <div class="d-block">
                                <span class="support support-box support-name hidden"></span>
                            </div>
                        </td>
                        <td class="td-updated_at">18/10/2020</td>
                        <td class="actions px-0">
                            <a href="#" class="btn btn-uno-transparent btn-icon mr-1" title="Actualizar">
                                <i class="icon fas fa-pen"></i>
                            </a>
                            <a href="#" class="btn btn-uno-transparent btn-icon mr-md-1" title="Borrar">
                                <i class="icon fas fa-trash"></i>
                            </a>
                        </td>
                    </tr>
                    <tr>
                        <td class="td-id_location" scope="row">3</td>
                        <td class="td-name">
                            <input type="text" name="name" id="name" placeholder="Nombre" value="pepe" disabled class="form-input p-2">
                            <div class="d-block">
                                <span class="support support-box support-name hidden"></span>
                            </div>
                        </td>
                        <td class="td-updated_at">18/10/2020</td>
                        <td class="actions px-0">
                            <a href="#" class="btn btn-uno-transparent btn-icon mr-1" title="Actualizar">
                                <i class="icon fas fa-pen"></i>
                            </a>
                            <a href="#" class="btn btn-uno-transparent btn-icon mr-md-1" title="Borrar">
                                <i class="icon fas fa-trash"></i>
                            </a>
                        </td>
                    </tr>
                </tbody> --}}
            </table>
        </main>
    </main>
</section>