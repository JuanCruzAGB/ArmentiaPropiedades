...
export function addImage(params){
    let key = document.querySelectorAll('.gallery .images .gallery-button img').length;
    for (const file of params.inputfilemaker.files) {
...
export function confirmImage(){
    ...
    if (regexp.exec(source)) {
        let input = document.querySelector(`.gallery .images .gallery-button img[src="${ source }"]`).previousElementSibling;
        input.checked = false;
...
function removeADomImage(route){
    for (const img of document.querySelectorAll('.gallery .images .gallery-button img')) {
        if (img.src == route) {
...
function createButton(){
    ...
    let inputFile = new InputFileMakerJS({
    ...
    div.appendChild(inputFile.getInput().getHTML());
    div.appendChild(inputFile.getButton().getHTML());
    div.appendChild(inputFile.getSpan().getHTML());
    document.querySelector('.images').insertBefore(div, document.querySelector('.images').children[0]);
}
...
function createLabel(key, route){
    let label = new HTMLCreatorJS('label', {
        properties: {
        id: `label-${ key }`,
        classes: ((key == 0) ? ['gallery-button', 'mb-3', 'selected'] : ['gallery-button', 'mb-3']),
    }, input: {
        properties: {
        id: `input-${ key }`,
        name: `files[${ key }]`,
